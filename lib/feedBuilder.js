...
const CORE_TAGS = [
  ...
  'category_id',
  'brand',
  'availability', // Тег уже есть в списке
  'weight',
  'height',
  'width',
  'length',
...
// Обновлен заголовок функции для приема inventoryMap
export function buildOffersXml(importValues, controlValues, deliveryValues, wixInventoryMap) {
...
// F: Units
  ...
  // Определяем индекс SKU в таблице (потребуется для lookup'а в wixInventoryMap)
  let skuColIndex = -1;
  const skuMapEntry = controlRows.find(row => {
      const feedName = row[idxFeedName];
      return feedName && String(feedName).trim() === 'sku';
  });

  if (skuMapEntry) {
      const skuFieldName = String(skuMapEntry[idxImportField]).trim();
      skuColIndex = headers.indexOf(skuFieldName);
  } else if (headers.indexOf('SKU') > -1) {
      // Фоллбэк, если в контрольном списке нет маппинга 'sku'
      skuColIndex = headers.indexOf('SKU');
  }
  ...
  rows.forEach((row) => {
    ...
    // **** ЛОГИКА: Определение availability на основе Wix стока ****
    let sku = '';
    let wixItem = null;
    
    if (skuColIndex > -1) {
      sku = row[skuColIndex] ? String(row[skuColIndex]).trim() : '';
      if (wixInventoryMap && sku) {
        wixItem = wixInventoryMap[sku];
      }
    }
    
    if (wixItem) {
        const isAvailable = wixItem.inStock === true;
        offerData.availability = isAvailable ? 'Є в наявності' : 'Немає в наявності';
    } else if (sku) {
        offerData.availability = 'Немає в наявності';
    }
    ...
    headers.forEach((header, colIndex) => {
      ...
      if (xmlName === 'availability' && wixItem) return;
