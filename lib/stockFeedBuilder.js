...
// Принимает cleanPrice, которая импортируется в monomarket-stock.js
export async function buildStockJson(importValues, controlValues, deliveryValues, getInventoryBySkus, cleanPrice) {
  ...
  const offersData = uniqueSkus.map(sku => {
    ...
    let oldPrice = rawData['old_price'] ? cleanPrice(rawData['old_price']) : null;
    ...
    // Собираем объект в правильном порядке
    const finalOffer = {
        code: sku,
        price: price,
        old_price: oldPrice,
  ...
  // .filter(o => o.price > 0); // <--- ЗАКОММЕНТИРОВАНО ДЛЯ ДИАГНОСТИКИ
