...
// === ДОБАВЛЕНИЕ ОПЛАТЫ (ВИПРАВЛЕНО СТРУКТУРУ) ===
export async function addExternalPayment(wixOrderId, amountStr, currency, createdDate) {
    ...
    // Wix вимагає 'regularPaymentDetails' для звичайних оплат.
    const payload = {
        payments: [
            {
                amount: { 
                ...
                // Важливо: поміщаємо деталі оплати сюди
                regularPaymentDetails: {
                    paymentMethod: "OFFLINE", // Вказуємо метод оплати
                    status: "APPROVED"        // Статус транзакції
                }
            }
        ]
    };
