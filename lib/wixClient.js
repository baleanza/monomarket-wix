...
// === ДОБАВЛЕНИЕ ОПЛАТЫ (Исправлено) ===
export async function addExternalPayment(wixOrderId, amountStr, currency, createdDate) {
    // Мы ожидаем, что amountStr уже приходит правильной строкой из Wix Order (например "123.00")
    ...
    console.log(`Adding PAYMENT for order ${wixOrderId}: ${amountStr} ${currency}`);
    ...
    try {
        ...
        if (!res.ok) {
            ...
            console.error(`!!! FAILED to add payment for ${wixOrderId}. Status: ${res.status}. Response: ${text}`);
            return null;
        }

        const data = await res.json();
        console.log(`Payment added successfully for ${wixOrderId}. Transaction ID: ${data.payment?.id}`);
        return data;
    } catch (e) {
...
export async function createWixRefund(orderId, paymentId, amount, currency) {
    const amountStr = String(amount); // Wix API любит строки
