...
export async function getWixOrderFulfillmentsBatch(orderIds) {
    ...
    try {
        ...
        // Wix возвращает объект { ordersWithFulfillments: [...] }
        const fulfillmentsList = [];
        if (data.ordersWithFulfillments) {
             data.ordersWithFulfillments.forEach(order => {
                // Преобразуем ответ для совместимости с mapWixOrderToMurkitResponse
                if (order.fulfillments && order.fulfillments.length > 0) {
                    order.fulfillments.forEach(f => {
                         // В каждой Fulfillment добавляем OrderId для идентификации
        ...
        // Эта функция должна возвращать список Fulfillments, поэтому она будет сложной, 
        // но для простоты мы оставим ее для использования внутри API.
