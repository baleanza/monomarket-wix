...
// === НОВА ФУНКЦІЯ: Оновлення деталей замовлення (Примітка та інше) ===
...
// === НОВА ФУНКЦІЯ: Додавання Зовнішньої Оплати (Щоб потім можна було зробити Refund) ===
export async function addExternalPayment(wixOrderId, amount, currency) {
    // Ця функція додає "Офлайн" оплату до замовлення.
    // Це створює транзакцію, яку пізніше можна "повернути".
    const payload = {
        payment: {
            amount: { amount: String(amount), currency: currency },
            type: "OFFLINE", // Позначаємо як зовнішню/офлайн оплату
            channel: "OTHER"
        }
    };

    console.log(`Adding EXTERNAL PAYMENT for order ${wixOrderId}: ${amount} ${currency}`);

    try {
        // Endpoint для додавання платежів до замовлення
        const res = await fetch(`${WIX_API_BASE}/ecom/v1/orders/${wixOrderId}/payments`, {
            method: 'POST',
            headers: getHeaders(),
            body: JSON.stringify(payload)
        });
        ...
        if (!res.ok) {
            ...
            console.error(`Failed to add payment for ${wixOrderId}:`, res.status, text);
            return null;
        }

        const data = await res.json();
        console.log(`Payment added successfully for ${wixOrderId}`);
        return data;
    } catch (e) {
        console.error("Network error adding payment:", e);
        return null;
    }
}
...
export async function createWixRefund(orderId, paymentId, amount, currency) {
    ...
    console.log(`Creating REFUND for order ${orderId}, payment ${paymentId}`);
