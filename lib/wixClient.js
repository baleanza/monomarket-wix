...
async function fetchAllProducts() {
  ...
  const limit = 100;
  let hasMore = true;
  const MAX_PAGES = 30; // Лимит страниц на всякий случай
  let page = 0;
...
export async function getProductsBySkus(skus) {
  ...
  // Нормализуем искомые SKU (строка + trim)
...
export async function getInventoryBySkus(skus) {
  ...
  // 1. Создаем Set из "чистых" SKU таблицы для быстрого поиска
  const targetSkus = new Set(skus.map(s => String(s).trim()));
  
  // 2. Скачиваем ВСЕ товары магазина
  const allProducts = await fetchAllProducts();
