...
// === [NEW] ДОБАВЛЕНИЕ ТРАНЗАКЦИИ ВОЗВРАТА (Option 1) ===
export async function addExternalRefundTransaction(wixOrderId, amountStr, currency, createdDate) {
    let transactionDate;
    try {
        transactionDate = createdDate ? new Date(createdDate).toISOString() : new Date().toISOString();
    } catch (e) {
    ...
    const negativeAmount = parseFloat(amountStr) * -1; // Делаем сумму отрицательной

    const payload = {
        payments: [
            {
                amount: { 
                    amount: String(negativeAmount.toFixed(2)), // Отрицательная сумма
                    currency: currency 
                },
                transactionDate: transactionDate,
                regularPaymentDetails: {
                    paymentMethod: "Monomarket Refund", // Используем другое название для ясности
                    status: "REFUNDED" // Устанавливаем статус "Возвращено"
                }
            }
        ]
    };
    ...
    console.log(`Adding REFUND transaction for order ${wixOrderId}, amount: ${negativeAmount.toFixed(2)}`);
    ...
    try {
        ...
        if (!res.ok) {
            console.error(`[DEBUG] !!! FAILED to add REFUND transaction. Status: ${res.status}. Response: ${text}`);
            return null;
        }
        console.log(`REFUND transaction successfully added for order ${wixOrderId}`);
        return JSON.parse(text);

    } catch (e) {
        console.error("[DEBUG] Network error adding REFUND transaction:", e);
        return null;
    }
}
...
export async function getWixPaymentDetails() { return null; } 
export async function createWixRefund() { 
    console.warn("createWixRefund is deprecated and removed.");
    return null;
} 
